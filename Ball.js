...
Ball.prototype.collideWithTable = function(table) {
    ...
    if (this.position.y <= table.TopY + CONSTANTS.ballRadius) {
        ...
        this.velocity = new Vector2(this.velocity.x, -this.velocity.y * 0.9); // Reverse vertical velocity
        collided = true;
    }

    if (this.position.x >= table.RightX - CONSTANTS.ballRadius) {
        this.position.x = table.RightX - CONSTANTS.ballRadius;
        this.velocity = new Vector2(-this.velocity.x * 0.9, this.velocity.y); // Reverse horizontal velocity
        collided = true;
    }
    ...
    if (collided) {
        // Adjust velocity to prevent continuous bouncing
